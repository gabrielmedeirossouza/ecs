...
import { StudentCreated, StudentCreatedResponseOk, StudentNotCreatedResponseBadRequest, StudentNotCreatedValidationFailed } from "@/components/student"
...
export class CreateStudentResponse implements System {
    ...
    @Write(StudentCreatedResponseOk, StudentNotCreatedResponseBadRequest, ResponseBadRequestRef, ResponseOkRef)
    execute({ view, buffer, world }: SystemContext) {
        for (const entity of view.query({ any: [StudentCreated, StudentNotCreatedValidationFailed] })) {
            ...
            if (entity.has(StudentCreated)) {
                buffer.add(entity, new StudentCreatedResponseOk("Estudante criado com sucesso!"))

                continue
            }

            buffer.add(entity, new StudentNotCreatedResponseBadRequest(entity.name))
        }
    }
}
